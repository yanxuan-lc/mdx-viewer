...
- **图内颜色的明暗适配**（`src/mdx/diagrams.mjs`，`dot` 与 `svg` 两车道共用；`mdxv` 与
  ...
  - **两个必须记住的配套约束**：① `<mask>`/`<clipPath>` 子树不参与语义化（那里的黑白是遮罩
    亮度语义不是颜色，`stroke` 同理），且要在容器上把 `fill="black"` 钉回去，否则根上的缺省
    色会继承进遮罩、把挖掉的区域重新显示出来——容器**自身**也要排除在语义化之外，不然刚钉
    回的黑又被当成「黑→前景」。② 根上的响应式尺寸声明要在 hast 层**与作者已有 `style` 合并**，
    不能在字符串上再拼一个 `style` 属性（两个同名属性 HTML 只保留第一个，作者的 style 会被
    静默吃掉）。
