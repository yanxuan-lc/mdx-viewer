...
| test-lane-invariant-unguarded | 三条测试车道的依赖表面不变式无人守卫 —— L1 混进 spawn / L2 混进构建时不会红,正是本次错标能潜伏数月的原因 | debt | P2 | agent | unconfirmed | — | — | package.json:test:unit/test:cli/test:build · 现状实测 L1 build=0 server=0、L2 build=0 server=4、L3 build=11,但这只是当下事实,没有断言钉住。可再验证判据:加一条测试,用 test/fixtures/vite-call-probe 探针分别跑三条车道,断言 test:unit 与 test:cli 的 build 计数为 0、test:unit 的 createServer 计数为 0;今天应绿,把任一构建断言塞回 L1/L2 时应红 |
